        ...
        canvas_.addGLEventListener(new GLEventListener() {
            ...
            public void reshape(
                ...
                if (!OS.isMacintosh()) {
					...
					newWidth = (int) (width * getHiDPIScalingFactor());
					newHeight = (int) (height * getHiDPIScalingFactor());
                }
  ...
  protected <T extends AWTEvent> T rescaleEvent(T anEvent) {
    ...
    public BufferedImage createScreenshot() {
    	if(SwingUtilities.isEventDispatchThread()) {
    		// The event dispatch thread must be available to perform OpenGL drawing
    		throw new RuntimeException("Screenshot capturing cannot be invoked from the event dispatch thread!");
    	}
    	
        int tempWidth = (int) (canvas_.getWidth() * getHiDPIScalingFactor());
        int tempHeight = (int) (canvas_.getHeight() * getHiDPIScalingFactor());
        final BufferedImage tempScreenshot = new BufferedImage(tempWidth, tempHeight, BufferedImage.TYPE_INT_ARGB);
			...
			public void display(GLAutoDrawable aDrawable) {
                ...
                for (int tempY = 0; tempY < tempHeight; tempY++) {
                    for (int tempX = 0; tempX < tempWidth; tempX++) {
                        ...
                        int tempR = (tempBuffer.get() & 0xff);
                        int tempG = (tempBuffer.get() & 0xff);
                        int tempB = (tempBuffer.get() & 0xff);
                        int tempA = (tempBuffer.get() & 0xff);
                        int tempARGB = (tempA << 24) + (tempR << 16) + (tempG << 8) + (tempB << 0);
                        tempScreenshot.setRGB(tempX, tempUseReadPixels ? (tempHeight  - tempY - 1) : tempY, tempARGB);
                    }
                }

                // Ready to notify the waiting thread.
